...
const buildProviderPayload = async (
  ...
  return {
    ...
    subject,
    // RFC 8058 one-click unsubscribe: the mail client POSTs `List-Unsubscribe=One-Click`
    // to the same signed URL the body link uses; that link's GET only shows a confirmation.
    headers: {
      'List-Unsubscribe': `<${unsubscribeUrl}>`,
      'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click',
    },
    html:`<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#13212E"><p style="font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#5B6670">GridOne winner update</p><h1 style="font-size:30px;line-height:1.1">${escapeHtml(heading)}</h1>${correctionDetail}<p style="font-size:17px">${escapeHtml(delivery.side_team)} ${delivery.side_digit} · ${escapeHtml(delivery.top_team)} ${delivery.top_digit}</p><p><a href="${boardUrl}" style="display:inline-block;background:#13212E;color:#F6F7F5;padding:14px 20px;text-decoration:none;font-weight:700">Open live board</a></p><p style="color:#5B6670;font-size:13px">GridOne tracks the board. It does not collect square money, hold funds, settle payments, or pay winners.</p><p style="font-size:12px"><a href="${unsubscribeUrl}" style="color:#5B6670">Stop winner emails for this board name</a></p></div>`,
  };
};
