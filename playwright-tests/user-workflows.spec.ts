...
test('published viewer renders the board and persists its canonical square selection', async ({ page }) => {
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('gridone:find-squares:ABCDEFGH') ?? 'null'))).toEqual({
...
test('organizer flushes the latest draft before publishing the viewer link', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Preview and publish', exact: true }).click();
  await page.getByRole('dialog', { name: 'Private preview — sharing is off' }).getByRole('button', { name: 'Review and publish', exact: true }).click();
