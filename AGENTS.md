...
- Number mode (Anthony-approved 2026-09-23): a board uses either one 0–9 top/side set for the whole game or new sets for each quarter (1st, 2nd, 3rd, Final; Final includes overtime). Every locked set has ten unique digits. Names and square positions never move between quarters. Winners for each milestone use that milestone's set, in the browser and in the database (`gridone_axis_for_milestone`). Never flatten one mode into the other or substitute another period's numbers.
