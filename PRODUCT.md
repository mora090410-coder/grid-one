...
Guest claiming is disabled unless server configuration explicitly allows a board. The original guest-claim feature has been released for a controlled test-board walkthrough. The participant sharing handoff described above is a subsequent local change; its new migration and deployment require their own release review. See `docs/guest-invites-operations.md` for the release boundary.
