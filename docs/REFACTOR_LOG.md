...
## 2026-09-26 — Shorter landing copy for every football-board organizer

- **Approval:** Anthony approved the copy plan and said “Implement”: broaden the audience beyond fundraisers, shorten repeated messaging, and emphasize the game-day experience.
- **Change:** “Football squares. Made easy.” hero with parties, office pools, friends, and fundraising as examples; three setup steps; next-score scenarios as the game-day benefit; reviewed Beta photo import behind a keyboard-operable disclosure; pricing cards without the repeated pricing paragraph; “Try the demo” links and “Run your board” footer. Broadened PRODUCT.md organizer description, sample-board identity, page metadata, no-JS copy, and loading fallback. Updated affected copy, routing, and browser assertions. Pricing and money boundaries are preserved.
- **Focused verification:** `npx vitest run --project unit tests/homepage tests/conversionPath.test.ts tests/pricingCopyConsistency.test.ts tests/productionCopy.test.ts tests/instrumentationWiring.test.tsx`: 8 files, 46 tests passed.
- **Gates:** `npx tsc --noEmit` passed; `npm run test:unit`: 159 files, 1200 tests passed; `npm run build` passed; `npm run design:lint`: 0 errors, 5 existing warnings. `PLAYWRIGHT_PORT=5199 npx playwright test --project=chromium`: 154 passed, 3 failed. Isolated rerun of organizer animation and quarter-number download passed; seller-link desktop selection contrast remains failing (Picked label, ratio 4.18 vs required 4.5), outside the edited landing surface. This is an unresolved gate, not a clean release.
- **Rendered verification:** inspected full rendered screenshots at 390px and 1280px; zero horizontal overflow; photo import opens with keyboard Enter; axe reports zero serious/critical homepage violations at both widths. Screenshots: `/tmp/gridone-copy-390.png` and `/tmp/gridone-copy-1280.png`. An initial screenshot captured the loading fallback; it was rejected and replaced after explicitly waiting for the homepage component.
- **Integration:** not run; `docker info` confirmed Docker daemon unavailable.
- **Delivery:** local changes only. Restored test-generated tracked screenshots under `.hermes/` and `.work/`. No commit, push, deployment, production change, or user contact.
