...
export function Footer() {
  return (
    <footer className="editorial-footer editorial-section">
      ...
      <div className="editorial-footer-guides"><div><Logo variant="horizontal" tone="reversed" size={32} className="editorial-footer-brand" /><Link to="/articles" className={ghostLink}>All guides</Link></div>{groups.map(group => <nav key={group.title} aria-label={group.title}><h2>{group.title}</h2><ul>{group.links.map(([slug, label]) => <li key={slug}><Link to={`/articles/${slug}`} className={ghostLink}>{label}</Link></li>)}</ul></nav>)}</div>
