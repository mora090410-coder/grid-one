...
export default function GuestInvitePrototype() {
  ...
  if (guestOnly) return <div className="min-h-[100dvh] bg-ground"><div role="status" className="m-4 rounded-control border border-gold bg-panel p-4 font-ui text-fg"><strong>Simulated prototype — no live database, payment, or account connections.</strong></div><GuestPoolPage transport={transport} /></div>;
  return <Base kind="cream"><main className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8">
    <div role="status" className="rounded-control border border-gold bg-panel p-4 font-ui text-fg"><strong>Simulated prototype — no live database, payment, or account connections.</strong></div>
